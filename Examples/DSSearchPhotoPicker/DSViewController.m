...
@interface DSViewController ()<UISearchBarDelegate, UISearchControllerDelegate,UISearchResultsUpdating, UISearchDisplayDelegate,UITableViewDelegate,UITableViewDataSource,gifSelected,imageSelected>{
...
- (void)viewDidLoad {
    ...
    viewA.gifDelegate=self;
    // Create the penalty view controller
 
   
     viewC = [self.storyboard instantiateViewControllerWithIdentifier:@"viewC"];
    viewC.imageDelegate=self;

    // Add A and B view controllers to the array
    self.allViewControllers = [[NSArray alloc] initWithObjects:viewA, viewC,nil];
...
- (void)searchBarCancelButtonClicked:(UISearchBar *)searchBar {
    ...
    viewA.passedResult = @"";
    viewC.passedResult = @"";
    viewA.imageArray= [NSMutableArray new];
    [viewA.collectionView reloadData];
    viewC.imageArray=[NSMutableArray new];
    viewC.contentArray=[NSMutableArray new];
...
-(void)openGif:(NSString *)passedId{
    NSLog(@"id recieved: %@",passedId);
    [self performSegueWithIdentifier:@"showImage" sender:passedId];
   }
-(void)openImage:(NSString *)passedURL{
     NSLog(@"url recieved: %@",passedURL);
    [self performSegueWithIdentifier:@"showImage" sender:passedURL];
   
}
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    if ([[segue identifier]isEqualToString:@"showImage"]) {
        DSImageView * controllers = (DSImageView *)segue.destinationViewController;
            controllers.recivedUrl=sender;
    }
}
