...
@interface DSThirdView ()<DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
{
    int pageNumber;
}
...
{
    NSString *text = @"Search Images here!";
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    activityLoader *loader = [_collectionView  dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionFooter
                 ...
                 cellForItemAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    [imageViewCell.imageCell sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",_imageArray[indexPath.item]]] placeholderImage:[UIImage imageNamed:@"placeHolder"] options:SDWebImageCacheMemoryOnly];
...
- (void)getImages:(int)pageId
{
    ...
    [manager GET:serviceUrl parameters:nil progress:nil success:^(NSURLSessionTask* task, id results) {
        ...
        NSDictionary *json = [NSJSONSerialization JSONObjectWithData:results options:NSJSONReadingMutableLeaves | NSJSONReadingAllowFragments error:nil];
         NSLog(@"image result: %@",json);
        
         NSArray * response  = [json valueForKeyPath:@"value.thumbnailUrl"];
          NSArray * contentResponse  = [json valueForKeyPath:@"value.contentUrl"];

        if (pageNumber == 0) {
            _loadingView.hidden=YES;
            _imageArray =[NSMutableArray arrayWithArray:response];
             _contentArray =[NSMutableArray arrayWithArray:contentResponse];
        ...
        [_contentArray addObjectsFromArray:contentResponse];
...
- (void)collectionView:(UICollectionView*)collectionView didSelectItemAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    NSString* passingUrl =[NSString stringWithFormat:@"%@",_contentArray[indexPath.item]];
    if([self.imageDelegate respondsToSelector:@selector(openImage:)]) {
        [self.imageDelegate openImage:passingUrl];
    }
